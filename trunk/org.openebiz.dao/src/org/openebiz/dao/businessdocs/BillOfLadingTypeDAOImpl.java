...
public class BillOfLadingTypeDAOImpl extends HibernateDaoSupport implements BillOfLadingTypeDAO {


	static private final Log log = LogFactory
			.getLog(BillOfLadingTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(BillOfLadingType.class);
	...
	public BillOfLadingType getById(Long id) {
		BillOfLadingType value = null;
		Session session = getSession();
		value = (BillOfLadingType) session.load(BillOfLadingType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(BillOfLadingType.class);
	...
	public BillOfLadingType merge(BillOfLadingType value) {
		BillOfLadingType newValue = null;
		Session session = getSession();
		newValue = (BillOfLadingType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		BillOfLadingType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
