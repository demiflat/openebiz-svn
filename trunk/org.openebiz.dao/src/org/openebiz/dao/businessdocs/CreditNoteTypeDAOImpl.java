...
import org.openebiz.core.businessdocs.CreditNoteType;
import java.util.Iterator;
import org.hibernate.criterion.Restrictions;
import org.hibernate.criterion.Order;
import org.hibernate.LockMode;

public class CreditNoteTypeDAOImpl extends HibernateDaoSupport implements CreditNoteTypeDAO {


	static private final Log log = LogFactory
			.getLog(CreditNoteTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(CreditNoteType.class);
	...
	public CreditNoteType getById(Long id) {
		CreditNoteType value = null;
		Session session = getSession();
		value = (CreditNoteType) session.load(CreditNoteType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(CreditNoteType.class);
	...
	public void insert(CreditNoteType value) {
		Session session = getSession();
		session.save(value);
	...
	public void update(CreditNoteType value) {
		Session session = getSession();
	...
	public CreditNoteType merge(CreditNoteType value) {
		CreditNoteType newValue = null;
		Session session = getSession();
		newValue = (CreditNoteType) session.merge(value);
		if (log.isDebugEnabled())
			log.debug("merging document: " + value);
		return newValue;
	}

	public void delete(CreditNoteType value) {
		Session session = getSession();
	...
	public void delete(Long id) {
		CreditNoteType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
