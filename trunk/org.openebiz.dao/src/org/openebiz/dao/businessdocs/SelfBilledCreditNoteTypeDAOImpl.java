...
public class SelfBilledCreditNoteTypeDAOImpl extends HibernateDaoSupport implements
		SelfBilledCreditNoteTypeDAO {


	static private final Log log = LogFactory
			.getLog(SelfBilledCreditNoteTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(SelfBilledCreditNoteType.class);
	...
	public SelfBilledCreditNoteType getById(Long id) {
		SelfBilledCreditNoteType value = null;
		Session session = getSession();
		value = (SelfBilledCreditNoteType) session.load(
				SelfBilledCreditNoteType.class, id);
		return value;
	}
	...
	public SelfBilledCreditNoteType merge(SelfBilledCreditNoteType value) {
		SelfBilledCreditNoteType newValue = null;
		Session session = getSession();
		newValue = (SelfBilledCreditNoteType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		SelfBilledCreditNoteType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
