...
public class DebitNoteTypeDAOImpl extends HibernateDaoSupport implements DebitNoteTypeDAO {


	static private final Log log = LogFactory
			.getLog(DebitNoteTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(DebitNoteType.class);
	...
	public DebitNoteType getById(Long id) {
		DebitNoteType value = null;
		Session session = getSession();
		value = (DebitNoteType) session.load(DebitNoteType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(DebitNoteType.class);
	...
	public DebitNoteType merge(DebitNoteType value) {
		DebitNoteType newValue = null;
		Session session = getSession();
		newValue = (DebitNoteType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		DebitNoteType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
