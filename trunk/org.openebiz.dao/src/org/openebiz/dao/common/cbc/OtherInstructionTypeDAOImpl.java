...
public class OtherInstructionTypeDAOImpl extends HibernateDaoSupport implements OtherInstructionTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(OtherInstructionTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(OtherInstructionType.class);
	...
	public OtherInstructionType getById(Long id) {
		OtherInstructionType value = null;
		Session session = getSession();
		value = (OtherInstructionType) session.load(OtherInstructionType.class,
				id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session
				.createCriteria(OtherInstructionType.class);
	...
	public OtherInstructionType merge(OtherInstructionType value) {
		OtherInstructionType newValue = null;
		Session session = getSession();
		newValue = (OtherInstructionType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		OtherInstructionType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
