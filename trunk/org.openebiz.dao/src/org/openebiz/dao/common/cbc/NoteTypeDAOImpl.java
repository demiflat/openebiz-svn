...
public class NoteTypeDAOImpl extends HibernateDaoSupport implements NoteTypeDAO {
	...
	static private final Log log = LogFactory.getLog(NoteTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(NoteType.class);
	...
	public NoteType getById(Long id) {
		NoteType value = null;
		Session session = getSession();
		value = (NoteType) session.load(NoteType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(NoteType.class);
	...
	public NoteType merge(NoteType value) {
		NoteType newValue = null;
		Session session = getSession();
		newValue = (NoteType) session.merge(value);
		if (log.isDebugEnabled())
