...
public class SpecialInstructionsTypeDAOImpl extends HibernateDaoSupport implements
		SpecialInstructionsTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(SpecialInstructionsTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(SpecialInstructionsType.class);
	...
	public SpecialInstructionsType getById(Long id) {
		SpecialInstructionsType value = null;
		Session session = getSession();
		value = (SpecialInstructionsType) session.load(
				SpecialInstructionsType.class, id);
		return value;
	}
	...
	public SpecialInstructionsType merge(SpecialInstructionsType value) {
		SpecialInstructionsType newValue = null;
		Session session = getSession();
		newValue = (SpecialInstructionsType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		SpecialInstructionsType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
