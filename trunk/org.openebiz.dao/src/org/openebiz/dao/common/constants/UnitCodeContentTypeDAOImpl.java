...
public class UnitCodeContentTypeDAOImpl extends HibernateDaoSupport implements UnitCodeContentTypeDAO {
		static private final Log log = LogFactory
			.getLog(UnitCodeContentTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(UnitCodeContentType.class);
	...
	public UnitCodeContentType getById(Long id) {
		UnitCodeContentType value = null;
		Session session = getSession();
		value = (UnitCodeContentType) session.load(UnitCodeContentType.class,
				id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session
				.createCriteria(UnitCodeContentType.class);
	...
	public UnitCodeContentType merge(UnitCodeContentType value) {
		UnitCodeContentType newValue = null;
		Session session = getSession();
		newValue = (UnitCodeContentType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		UnitCodeContentType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
