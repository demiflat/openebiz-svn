...
public class LanguageCodeContentTypeDAOImpl extends HibernateDaoSupport implements
		LanguageCodeContentTypeDAO {
		static private final Log log = LogFactory
			.getLog(LanguageCodeContentTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(LanguageCodeContentType.class);
	...
	public LanguageCodeContentType getById(Long id) {
		LanguageCodeContentType value = null;
		Session session = getSession();
		value = (LanguageCodeContentType) session.load(
				LanguageCodeContentType.class, id);
		return value;
	}
	...
	public LanguageCodeContentType merge(LanguageCodeContentType value) {
		LanguageCodeContentType newValue = null;
		Session session = getSession();
		newValue = (LanguageCodeContentType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		LanguageCodeContentType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
