...
public class BinaryObjectTypeDAOImpl extends HibernateDaoSupport implements BinaryObjectTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(BinaryObjectTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(BinaryObjectType.class);
	...
	public BinaryObjectType getById(Long id) {
		BinaryObjectType value = null;
		Session session = getSession();
		value = (BinaryObjectType) session.load(BinaryObjectType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(BinaryObjectType.class);
		...
		if (criteria.getCharacterSetCode() != null) {
			newCriteria.add(Restrictions.eq("characterSetCode", criteria
					.getCharacterSetCode()));
		}
		if ((criteria.ascending != null)
				&& (criteria.ascending.contains("characterSetCode")))
			newCriteria.addOrder(Order.asc("characterSetCode"));
		else if ((criteria.descending != null)
				&& (criteria.descending.contains("characterSetCode")))
			newCriteria.addOrder(Order.desc("characterSetCode"));
	...
	public void insert(BinaryObjectType value) {
		Session session = getSession();
		session.save(value);
	...
	public void update(BinaryObjectType value) {
		Session session = getSession();
	...
	public BinaryObjectType merge(BinaryObjectType value) {
		BinaryObjectType newValue = null;
		Session session = getSession();
		newValue = (BinaryObjectType) session.merge(value);
		if (log.isDebugEnabled())
			log.debug("merging document: " + value);
		return newValue;
	}

	public void delete(BinaryObjectType value) {
		Session session = getSession();
	...
	public void delete(Long id) {
		BinaryObjectType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
