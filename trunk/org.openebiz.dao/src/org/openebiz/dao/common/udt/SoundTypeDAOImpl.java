...
public class SoundTypeDAOImpl extends HibernateDaoSupport implements SoundTypeDAO {

	
	static private final Log log = LogFactory.getLog(SoundTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(SoundType.class);
	...
	public SoundType getById(Long id) {
		SoundType value = null;
		Session session = getSession();
		value = (SoundType) session.load(SoundType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(SoundType.class);
	...
	public SoundType merge(SoundType value) {
		SoundType newValue = null;
		Session session = getSession();
		newValue = (SoundType) session.merge(value);
		if (log.isDebugEnabled())
