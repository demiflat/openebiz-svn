...
public class VideoTypeDAOImpl extends HibernateDaoSupport implements VideoTypeDAO {

	
	static private final Log log = LogFactory.getLog(VideoTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(VideoType.class);
	...
	public VideoType getById(Long id) {
		...
		Session session = getSession();
		value = (VideoType) session.load(VideoType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(VideoType.class);
	...
	public VideoType merge(VideoType value) {
		VideoType newValue = null;
		Session session = getSession();
		newValue = (VideoType) session.merge(value);
		if (log.isDebugEnabled())
