...
public class ItemPropertyGroupTypeDAOImpl extends HibernateDaoSupport implements ItemPropertyGroupTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(ItemPropertyGroupTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(ItemPropertyGroupType.class);
	...
	public ItemPropertyGroupType getById(Long id) {
		ItemPropertyGroupType value = null;
		Session session = getSession();
		value = (ItemPropertyGroupType) session.load(
				ItemPropertyGroupType.class, id);
		return value;
	}
	...
	public ItemPropertyGroupType merge(ItemPropertyGroupType value) {
		ItemPropertyGroupType newValue = null;
		Session session = getSession();
		newValue = (ItemPropertyGroupType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		ItemPropertyGroupType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
