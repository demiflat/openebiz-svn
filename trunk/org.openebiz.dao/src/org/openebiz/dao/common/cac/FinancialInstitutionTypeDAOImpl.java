...
public class FinancialInstitutionTypeDAOImpl extends HibernateDaoSupport implements
		FinancialInstitutionTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(FinancialInstitutionTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(FinancialInstitutionType.class);
	...
	public FinancialInstitutionType getById(Long id) {
		FinancialInstitutionType value = null;
		Session session = getSession();
		value = (FinancialInstitutionType) session.load(
				FinancialInstitutionType.class, id);
		return value;
	}
	...
	public FinancialInstitutionType merge(FinancialInstitutionType value) {
		FinancialInstitutionType newValue = null;
		Session session = getSession();
		newValue = (FinancialInstitutionType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		FinancialInstitutionType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
