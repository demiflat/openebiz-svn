...
public class PortTypeDAOImpl extends HibernateDaoSupport implements PortTypeDAO {

	
	static private final Log log = LogFactory.getLog(PortTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(PortType.class);
	...
	public PortType getById(Long id) {
		PortType value = null;
		Session session = getSession();
		value = (PortType) session.load(PortType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(PortType.class);
		...
		if (criteria.getLocation() != null) {
			newCriteria
					.add(Restrictions.eq("location", criteria.getLocation()));
		}
		if ((criteria.ascending != null)
				&& (criteria.ascending.contains("location")))
			newCriteria.addOrder(Order.asc("location"));
		else if ((criteria.descending != null)
				&& (criteria.descending.contains("location")))
			newCriteria.addOrder(Order.desc("location"));
	...
	public PortType merge(PortType value) {
		PortType newValue = null;
		Session session = getSession();
		newValue = (PortType) session.merge(value);
		if (log.isDebugEnabled())
