...
public class DocumentReferenceTypeDAOImpl extends HibernateDaoSupport implements DocumentReferenceTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(DocumentReferenceTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(DocumentReferenceType.class);
	...
	public DocumentReferenceType getById(Long id) {
		DocumentReferenceType value = null;
		Session session = getSession();
		value = (DocumentReferenceType) session.load(
				DocumentReferenceType.class, id);
		return value;
	}
	...
	public void insert(DocumentReferenceType value) {
		Session session = getSession();
		session.save(value);
	...
	public void update(DocumentReferenceType value) {
		Session session = getSession();
	...
	public DocumentReferenceType merge(DocumentReferenceType value) {
		DocumentReferenceType newValue = null;
		Session session = getSession();
		newValue = (DocumentReferenceType) session.merge(value);
		if (log.isDebugEnabled())
			log.debug("merging document: " + value);
		return newValue;
	}

	public void delete(DocumentReferenceType value) {
		Session session = getSession();
	...
	public void delete(Long id) {
		DocumentReferenceType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
