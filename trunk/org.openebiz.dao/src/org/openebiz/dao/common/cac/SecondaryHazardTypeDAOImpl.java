...
public class SecondaryHazardTypeDAOImpl extends HibernateDaoSupport implements SecondaryHazardTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(SecondaryHazardTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(SecondaryHazardType.class);
	...
	public SecondaryHazardType getById(Long id) {
		SecondaryHazardType value = null;
		Session session = getSession();
		value = (SecondaryHazardType) session.load(SecondaryHazardType.class,
				id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session
				.createCriteria(SecondaryHazardType.class);
	...
	public SecondaryHazardType merge(SecondaryHazardType value) {
		SecondaryHazardType newValue = null;
		Session session = getSession();
		newValue = (SecondaryHazardType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		SecondaryHazardType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
