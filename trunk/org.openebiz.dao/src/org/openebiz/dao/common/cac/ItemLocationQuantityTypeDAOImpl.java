...
public class ItemLocationQuantityTypeDAOImpl extends HibernateDaoSupport implements
		ItemLocationQuantityTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(ItemLocationQuantityTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(ItemLocationQuantityType.class);
	...
	public ItemLocationQuantityType getById(Long id) {
		ItemLocationQuantityType value = null;
		Session session = getSession();
		value = (ItemLocationQuantityType) session.load(
				ItemLocationQuantityType.class, id);
		return value;
	}
	...
	public ItemLocationQuantityType merge(ItemLocationQuantityType value) {
		ItemLocationQuantityType newValue = null;
		Session session = getSession();
		newValue = (ItemLocationQuantityType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		ItemLocationQuantityType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
