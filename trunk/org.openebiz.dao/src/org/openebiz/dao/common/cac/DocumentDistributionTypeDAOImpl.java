...
public class DocumentDistributionTypeDAOImpl extends HibernateDaoSupport implements
		DocumentDistributionTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(DocumentDistributionTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(DocumentDistributionType.class);
	...
	public DocumentDistributionType getById(Long id) {
		DocumentDistributionType value = null;
		Session session = getSession();
		value = (DocumentDistributionType) session.load(
				DocumentDistributionType.class, id);
		return value;
	}
	...
	public DocumentDistributionType merge(DocumentDistributionType value) {
		DocumentDistributionType newValue = null;
		Session session = getSession();
		newValue = (DocumentDistributionType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		DocumentDistributionType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
