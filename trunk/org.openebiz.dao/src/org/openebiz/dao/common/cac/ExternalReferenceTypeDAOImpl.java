...
public class ExternalReferenceTypeDAOImpl extends HibernateDaoSupport implements ExternalReferenceTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(ExternalReferenceTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session
				.createCriteria(ExternalReferenceType.class);
	...
	public ExternalReferenceType getById(Long id) {
		ExternalReferenceType value = null;
		Session session = getSession();
		value = (ExternalReferenceType) session.load(
				ExternalReferenceType.class, id);
		return value;
	}
	...
	public ExternalReferenceType merge(ExternalReferenceType value) {
		ExternalReferenceType newValue = null;
		Session session = getSession();
		newValue = (ExternalReferenceType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		ExternalReferenceType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
