...
public class PriceListTypeDAOImpl extends HibernateDaoSupport implements PriceListTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(PriceListTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(PriceListType.class);
	...
	public PriceListType getById(Long id) {
		PriceListType value = null;
		Session session = getSession();
		value = (PriceListType) session.load(PriceListType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(PriceListType.class);
		...
		if (criteria.getPreviousPriceList() != null) {
			newCriteria.add(Restrictions.eq("previousPriceList", criteria
					.getPreviousPriceList()));
		}
		if ((criteria.ascending != null)
				&& (criteria.ascending.contains("previousPriceList")))
			newCriteria.addOrder(Order.asc("previousPriceList"));
		else if ((criteria.descending != null)
				&& (criteria.descending.contains("previousPriceList")))
			newCriteria.addOrder(Order.desc("previousPriceList"));
	...
	public PriceListType merge(PriceListType value) {
		PriceListType newValue = null;
		Session session = getSession();
		newValue = (PriceListType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		PriceListType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
