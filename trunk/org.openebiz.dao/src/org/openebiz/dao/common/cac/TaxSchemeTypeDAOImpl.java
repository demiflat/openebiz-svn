...
public class TaxSchemeTypeDAOImpl extends HibernateDaoSupport implements TaxSchemeTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(TaxSchemeTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(TaxSchemeType.class);
	...
	public TaxSchemeType getById(Long id) {
		TaxSchemeType value = null;
		Session session = getSession();
		value = (TaxSchemeType) session.load(TaxSchemeType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(TaxSchemeType.class);
		...
		if (criteria.getTaxTypeCode() != null) {
			newCriteria.add(Restrictions.eq("taxTypeCode", criteria
					.getTaxTypeCode()));
		}
		if ((criteria.ascending != null)
				&& (criteria.ascending.contains("taxTypeCode")))
			newCriteria.addOrder(Order.asc("taxTypeCode"));
		else if ((criteria.descending != null)
				&& (criteria.descending.contains("taxTypeCode")))
			newCriteria.addOrder(Order.desc("taxTypeCode"));
	...
	public TaxSchemeType merge(TaxSchemeType value) {
		TaxSchemeType newValue = null;
		Session session = getSession();
		newValue = (TaxSchemeType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		TaxSchemeType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
