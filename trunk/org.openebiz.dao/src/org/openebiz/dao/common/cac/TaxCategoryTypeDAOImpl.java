...
public class TaxCategoryTypeDAOImpl extends HibernateDaoSupport implements TaxCategoryTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(TaxCategoryTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(TaxCategoryType.class);
	...
	public TaxCategoryType getById(Long id) {
		TaxCategoryType value = null;
		Session session = getSession();
		value = (TaxCategoryType) session.load(TaxCategoryType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(TaxCategoryType.class);
	...
	public TaxCategoryType merge(TaxCategoryType value) {
		TaxCategoryType newValue = null;
		Session session = getSession();
		newValue = (TaxCategoryType) session.merge(value);
		if (log.isDebugEnabled())
	...
	public void delete(Long id) {
		TaxCategoryType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
