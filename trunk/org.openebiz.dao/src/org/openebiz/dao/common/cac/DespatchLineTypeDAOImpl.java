...
public class DespatchLineTypeDAOImpl extends HibernateDaoSupport implements DespatchLineTypeDAO {

	
	static private final Log log = LogFactory
			.getLog(DespatchLineTypeDAOImpl.class);

	public List getAll() {
		List values = null;
		Session session = getSession();
		Criteria allValues = session.createCriteria(DespatchLineType.class);
	...
	public DespatchLineType getById(Long id) {
		DespatchLineType value = null;
		Session session = getSession();
		value = (DespatchLineType) session.load(DespatchLineType.class, id);
		return value;
	}
	...
	private Criteria getHBCriteria(Session session, SearchCriteria criteria) {
		Criteria newCriteria = session.createCriteria(DespatchLineType.class);
	...
	public void insert(DespatchLineType value) {
		Session session = getSession();
		session.save(value);
	...
	public void update(DespatchLineType value) {
		Session session = getSession();
	...
	public DespatchLineType merge(DespatchLineType value) {
		DespatchLineType newValue = null;
		Session session = getSession();
		newValue = (DespatchLineType) session.merge(value);
		if (log.isDebugEnabled())
			log.debug("merging document: " + value);
		return newValue;
	}

	public void delete(DespatchLineType value) {
		Session session = getSession();
	...
	public void delete(Long id) {
		DespatchLineType value = getById(id);
		Session session = getSession();
		if (value != null) {
			session.refresh(value, LockMode.UPGRADE);
			session.delete(value);
